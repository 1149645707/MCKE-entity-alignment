...
class StructureEmbed(nn.Module):
    ...
    def forward(self, sr_ent_seeds, tg_ent_seeds):
        ...
        # print(sr_ent_hid.shape) #以zh/en为例,torch.Size([19388, 128]) 表示将zh 中的19388个实体每个实体嵌入表示为128维的向量
        # print(tg_ent_hid.shape) #torch.Size([19572, 128])
        # print(sr_ent_seeds.shape)#以zh/en为例,torch.Size([112500, 2])
        # print(tg_ent_seeds.shape)  #torch.Size([112500, 2])
        # sr_ent_hid表示source KG 所有实体的向量表示,sr_seed_hid表示source KG 中所有实体对的向量表示

        sr_seed_hid = sr_ent_hid[sr_ent_seeds.type(torch.long)]
        tg_seed_hid = tg_ent_hid[tg_ent_seeds.type(torch.long)]
        # print(sr_seed_hid.shape) #torch.Size([112500, 2, 128])
        return sr_seed_hid, tg_seed_hid, sr_ent_hid, tg_ent_hid
...
class AttributeEmbed(nn.Module):
    def __init__(self, dim,attention_property_vec_sr,attention_property_vec_tg,entity_vec_sr,entity_vec_tg,value_vec_sr,value_vec_tg,property_vec_sr,property_vec_tg):
        ...
        W_sr = torch.ones((1, 768), dtype=torch.float)  # (1,768)
        ...
        indim = 768 + 2 * dim + dim
        self.linear = nn.Linear(indim, dim)    #if dim=128, then 1152; if dim=64,then 960
    ...
    def forward(self, sr_ent_seeds, tg_ent_seeds):
        entity_vec_sr = self.Weight_Attribute_sr * self.entity_vec_sr
        entity_vec_tg = self.Weight_Attribute_tg * self.entity_vec_tg

        entity_vec_sr_fc = self.proj_sr(entity_vec_sr)
        value_vec_sr = self.proj_sr(self.value_vec_sr)
        property_vec_sr = self.proj_sr(self.property_vec_sr)
        attention_property_vec_sr = self.proj_sr(self.attention_property_vec_sr)  # (N, 256)

        entity_vec_tg_fc = self.proj_tg(entity_vec_tg)
        value_vec_tg = self.proj_tg(self.value_vec_tg)
        property_vec_tg = self.proj_tg(self.property_vec_tg)
        attention_property_vec_tg = self.proj_tg(self.attention_property_vec_tg)  # (M, 256)

        exp_out_sr = self.alpha * entity_vec_sr_fc * property_vec_sr + self.beta * entity_vec_sr_fc * value_vec_sr
        exp_out_sr = self.dropout(exp_out_sr)
        exp_out_sr = exp_out_sr.unsqueeze(1)
        exp_out_sr = self.pool(exp_out_sr) * 2
        exp_out_sr = torch.sqrt(F.relu(exp_out_sr)) - torch.sqrt(F.relu(-exp_out_sr))
        exp_out_sr = F.normalize(exp_out_sr.squeeze())  # (N, 128)

        exp_out_tg = self.alpha * entity_vec_tg_fc * property_vec_tg + self.beta * entity_vec_tg_fc * value_vec_tg
        exp_out_tg = self.dropout(exp_out_tg)
        exp_out_tg = exp_out_tg.unsqueeze(1)
        exp_out_tg = self.pool(exp_out_tg) * 2
        exp_out_tg = torch.sqrt(F.relu(exp_out_tg)) - torch.sqrt(F.relu(-exp_out_tg))
        exp_out_tg = F.normalize(exp_out_tg.squeeze())  # (M, 128)
...
class MCKE(object):
    ...
    def __init__(self):
        ...
        self.dim = 128  #N A S 128
        self.drop_out = 0.0
        self.layer_num = 2
        self.epoch_num = 2000 #S 400  N A 3000
        self.nega_sample_freq = 5
        self.nega_sample_num = 25  # DBP15K 50  DWY100K 25
        ...
        self.coIter_att_num = 1  # DBP15K 4  DWY100K 1
    ...
    def init(self, directory, device):
        set_random_seed()
        self.directory = Path(directory)  # eg: directory:'./bin/DBP15k/zh_en'
...
def MCKE_EA(log_comment, data_set, layer_num, device, load_new_seed_split=False, save_model=False):
    ...
    nsa_embed.set_channel(log_comment)  # log_comment： Name/Structure/Attribute
    ...
    if log_comment == 'Name':
        ...
        nsa_embed.init_log(nsa_embed.log_dir)
        _, _, training_info =nsa_embed.train_StructureEmbed(device)  #train_batch
